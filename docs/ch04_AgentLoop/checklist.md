...
## 端到端场景
- [ ] 场景 1（多轮连环）：openai 兼容端点 → 「读 `docs/ch03_ToolSystem/spec.md`，再据内容新建 `docs/ch03_ToolSystem/summary.txt` 写一句话摘要」→ read_file → write_file 跨多轮自动出现 → 状态栏用量增长、动态区轮次递增 → 最终答复 → `/exit` 无残留。
