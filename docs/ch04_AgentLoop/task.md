...
4. `Agent.java`：
   ...
   - `streamOnce(conv, defs, suffix, cancel, bus) → StreamOutcome`：`suffix` 为 ch04_AgentLoop 新增形参，透传给 `provider.stream`；订阅 `Flow.Publisher<llm.StreamEvent>` 同步消费——`switch` pattern match 处理 `TextDelta` / `ToolCalls` / `UsageEvent` / `Done` / `Failed`；记录 `usage`、收集 `calls`、转发 Text，`Failed` 即发 `Event.Failed` 返回 `failed=true`。
   ...
   - 辅助：`allUnknown(calls)`（每个 call 用 `registry.get` 判，全未注册才 true）、`ensureFinal`（沿用 ch03_ToolSystem）、`ensureAssistantTail(conv, fallback)`、`finishCancelled(conv)`、`emit` / `argsPreview`（沿用 ch03_ToolSystem）。
5. `AgentTest.java`（**替换** ch03_ToolSystem 的 `testSingleRoundReadAndAnswer` / `testSingleRoundLimit`——后者断言单轮已与 ch04_AgentLoop 多轮矛盾）。`FakeProvider#stream` 签名补 `String systemSuffix`（并在某用例里记录收到的 `tools` / `suffix` 供断言）；多轮靠 `List<List<StreamEvent>> scripts` 逐次返回：
...
2. 端到端（openai 兼容端点，用 `.mewcode/config.yaml`）：
   - 多轮（AC1）：问「读 `docs/ch03_ToolSystem/spec.md`，再据其内容新建 `docs/ch03_ToolSystem/summary.txt` 写一句话摘要」→ 观察 read_file → write_file 跨多轮自动连环、状态栏用量增长、动态区轮次递增、最终答复。
