...
ch04_AgentLoop 不新增包，在 ch03_ToolSystem「tool / agent / llm / conversation / prompt / tui」之上**扩展**：

- **dev.mewcode.agent（重写 `Agent.run`）**：把 ch03_ToolSystem 的「请求#1 → 执行 → 请求#2 → 停」改为真正的 ReAct 循环——`for` 迭代直到自然完成 / 上限 / 取消 / 连续未知工具 / 出错。新增保序分批并发执行、迭代进度与用量事件、终止时的历史一致性收尾、Plan/Normal 两种模式。
...
`Message`/`ToolCall`/`ToolResult`/`ToolDefinition` 与 `Role.TOOL` 沿用 ch03_ToolSystem，不变。
...
public sealed interface Event
    ...
    record Tool(ToolEvent toolEvent) implements Event {}           // 工具调用开始/结束（沿用 ch03_ToolSystem）
...
`ToolEvent`、`Phase`(START/END)、`Agent`、`Agent(provider, registry)` 构造沿用 ch03_ToolSystem。`run` 签名新增 `mode` 形参与 per-turn `CancelToken`。

`Agent` 构造沿用 ch03_ToolSystem：`public Agent(Provider provider, Registry registry)`。`mode` 为 `run` 的每次调用入参，不写入 `Agent` 状态（同一 `Agent` 可被不同 mode 复用）。
...
- 单个串行工具：`emit(Event.Tool{Start})` → 执行 → `emit(Event.Tool{End})`（沿用 ch03_ToolSystem 时序，动态区显示该工具 Running）。
...
**辅助函数：**
- `emit(bus, event) → boolean`：沿用 ch03_ToolSystem——`if (cancel.isCancelled()) return false; bus.submit(event); return true;`。返回 false 当且仅当 per-turn cancel 被触发（`SubmissionPublisher` 由 `run` 自己持有并 try-with-resources 关闭，不会在提交中被外部关）。调用方据 false 提前收尾。
...
- `ensureFinal(bus, text)`：沿用 ch03_ToolSystem——`text` 非空原样返回；为空则 emit 占位提示并返回占位文本（避免空 assistant 回合破坏下一轮请求）。
...
**职责：** 协议无关请求/响应 + 两协议工具调用全流程（沿用 ch03_ToolSystem）+ 本轮用量上抛（F8）+ 系统后缀（F10）。
...
- 历史含工具交互时 thinking 已自动关闭（ch03_ToolSystem 既有逻辑），多轮续答沿用，无需改动。
...
`SYSTEM_PROMPT` 增补一句 Agent 循环约定（追加到现有文案）：`"Keep using tools across multiple steps to make progress, and only give your final concise answer once the task is complete."`（中文项目里保持英文 system prompt 风格，与 ch03_ToolSystem 现有 `SYSTEM_PROMPT` 一致）。
...
- `List<ToolDisplay> curTools`——替换 ch03_ToolSystem 的单个 `ToolDisplay curTool`，支持并发批多个在执行的工具行。
...
- `toolLine` / `toolResultSummary` 沿用 ch03_ToolSystem。
...
> 注：`Main.java` 已在 ch03_ToolSystem 注入 `Registry`，ch04_AgentLoop 无需改动；`mode` 状态存于 `TuiApp`，不经 `Main`。

### 签名变更的调用方清单（实测核对，确保编译不漏）

ch04_AgentLoop 改了两个签名，必须同步所有调用方/实现方，否则编译断：
...
| 不用 SDK 内置 tool-runner | 坚持手写循环 + stable streaming | 沿用 ch03_ToolSystem 决策；自写循环才能精确控制停止条件、保序分批、取消与历史收尾，SDK 的自动 runner 把这些黑盒化。 |
...
| 迭代上限值 | `MAX_ITERATIONS=25`，内置常量 | 兜底安全网，避免失控烧 token；25 足够覆盖正常多步任务。spec 明确不配置化，与 ch03_ToolSystem 超时不配化一致。 |
...
| 进度事件 | 每轮起始 emit `Event.Iter(n)`，UI 显示「第 N 轮」 | F9 让用户感知多轮推进；用 sealed 子类型分派，与 ch03_ToolSystem 的事件惯例一致。 |
