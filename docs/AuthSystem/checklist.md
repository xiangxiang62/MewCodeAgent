...
- [ ] 不破坏 AgentLoop/SystemManager：多轮连环、用户取消、流出错恢复、历史一致、缓存命中、规划按轮次注入仍成立（验证：跑既有端到端关键场景；`mvn test` 通过）。(AC16/N3)
...
- [ ] 场景 8（plan 不变）：`/plan` 仅放只读工具产出计划、`/do` 执行——沿用 SystemManager 行为不退化。(AC9/F7)
