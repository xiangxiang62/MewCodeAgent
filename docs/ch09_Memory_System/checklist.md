...
- [ ] buildSystemPrompt 向后兼容：传入空字符串 → 输出与 ch08_Context_Manager 完全一致（验证：对比 ch08_Context_Manager 的 `buildSystemPrompt()` 输出和新 `buildSystemPrompt("", "")` 输出，逐字节一致）
- [ ] Conversation 回调向后兼容：使用无参 `new Conversation()` 构造 → 所有 add/replace 行为不变（验证：ch08_Context_Manager 的 conversation 测试全部通过）
...
- [ ] 场景 4（会话存档 + 工具记录）：启动 → 输入 "读取 pom.xml" → 模型调 read_file → 回复内容；`cat .mewcode/sessions/*/conversation.jsonl` 至少 4 行（user → assistant+tool_calls → tool_results → assistant）；`grep tool_calls` 和 `grep tool_results` 各至少一条命中。(AC8/AC9)
