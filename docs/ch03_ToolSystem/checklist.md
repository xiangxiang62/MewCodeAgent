...
- [ ] read_file 带行号读出内容；读不存在/目录返回结构化错误（验证：单测 + 手测读 `docs/ch03_ToolSystem/spec.md` 见行号、读不存在文件得 isError）。(AC2/F2)
...
## 端到端场景
- [ ] 场景 1（读文件并总结）：openai 兼容端点 → 问「读 docs/ch03_ToolSystem/spec.md 用一句话总结」→ `● read_file(...)` 工具行 + 结果摘要 + 最终 markdown 答复 → /exit 退出，终端无残留。
