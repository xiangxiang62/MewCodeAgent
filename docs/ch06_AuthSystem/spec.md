...
ch05_SystemManager 做完系统提示工程化后，MewCode 已能自主多轮想 → 调工具 → 看结果地干活。但**工具执行是完全裸奔的**：工具执行链路上没有任何闸门——写文件工具能写任意路径、改文件工具能改任意文件、命令执行能跑任意命令（含 `rm -rf /`）、读写类工具能触碰项目目录之外的任何位置。这在 ch01_Multi-turn 背景里就被列为预留能力（「在没有工具调用、**权限**、记忆等高级能力之前」）。

ch06_AuthSystem 给 MewCode 装上**五层防御的权限系统**：危险命令黑名单硬拦截、路径沙箱限定操作范围、可配置规则做细粒度放行/拦截、多档权限模式覆盖信任等级、人在回路作为最后确认。五层串联、逐层短路，一次工具调用在真正执行前必须穿过这道护栏。权限被拒绝时把结构化拒绝**回灌**给模型让它调整策略，而非终止 Agent Loop。这是从「会自己干」到「在安全护栏内自己干」的关键一跃。
...
- F5: 权限模式与按类别兜底
  ...
  plan 档额外沿用 ch04_AgentLoop 行为：仅向模型暴露只读工具定义 + 注入计划提醒，文件写 / 命令执行压根不会被请求（矩阵中 plan 行的 Ask 仅为防御性兜底，即便该类工具被请求也判 Ask）。
...
- N3: 不破坏 ch04_AgentLoop/ch05_SystemManager——Agent Loop 多轮编排、流式双路收集、**保序分批并发**、用户取消、历史一致、缓存与 system-reminder 注入等既有能力不退化；只读工具的并发执行不因权限检查退化为串行（只读永不触发 Ask）。
...
- AC16: 不破坏 ch04_AgentLoop/ch05_SystemManager——多轮连环、用户取消、流出错恢复、历史一致、缓存命中、规划按轮次注入仍成立；`mvn test` 通过。（N3）
