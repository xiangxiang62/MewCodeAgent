...
**依赖：** T8
**步骤：**
1. 既有 ch04_AgentLoop/ch05_SystemManager 用例：`new Agent(...)` 增 `engine` 实参（`PermissionEngine.create(tempDir)`）；`Mode.NORMAL`→`Mode.DEFAULT`；fake `Provider` 签名不变。
2. 新增：
   ...
   - **plan 迁移**：`Mode.PLAN` 仍只放只读工具、注入计划提醒（沿用 ch05_SystemManager 断言，枚举换名）。
