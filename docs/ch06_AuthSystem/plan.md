...
ch06_AuthSystem 新增一个 **permission 包**承载前四层防御与配置加载，并在 **agent 包**把判定接入工具执行链、由 agent 编排承载第五层人在回路；**tui 包**新增「待批准」交互态承载人在回路的 UI；**Main** 负责装配引擎并注入。**不改 llm / provider 适配层**（N6 跨协议一致天然成立）。
...
public enum Mode {
    ...
    PLAN,           // 仅只读工具可见（沿用 ch04_AgentLoop）；矩阵同 default 作防御兜底
...
// AgentEvent（原 ch04_AgentLoop sealed interface StreamEvent 的兄弟）新增成员：
...
| plan 语义 | 沿用 ch04_AgentLoop 硬限制（只读工具集+提醒）+ /do | 用户拍板；矩阵 plan 行仅防御性兜底；/plan 与 defaultMode=plan 都按 Mode.PLAN 应用 |
...
| 只读权限检查 | 批内逐个 check，但只读永不 Ask | N3：保留 ch04_AgentLoop 并发；只读最多被沙箱/deny 规则拦为 Deny，无需交互 |
...
| ToolCall.input 解析 | Jackson `ObjectMapper.readTree` | ch04_AgentLoop 的 ToolCall 已以 JSON 文本承载参数；Jackson 已在 SDK 依赖里出现（openai-java 用），加显式依赖更稳；不引入 gson 等额外库 |
